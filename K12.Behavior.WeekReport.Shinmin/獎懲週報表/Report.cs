...
namespace K12.Behavior.WeekReport.Shinmin.獎懲週報表
{
    internal class Report : IReport
    {
        ...
        void _BGWDisciplineWeekList_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            prototype.Worksheets[0].Cells.Merge(titleRow, dayStartIndex, 3, 1);
            ...
            foreach (ClassRecord classInfo in selectedClass)
            {
                if (list.Contains(classInfo.ID))
                {
                    ...
                    //填寫基本資料
                    string TitleName1 = K12.Data.School.DefaultSchoolYear + " 學年度 " + School.DefaultSemester + " 學期 " + School.ChineseName + " 獎懲週報表";

                    string TitleName2 = "";
                    string tname = classInfo.Teacher == null ? "" : classInfo.Teacher.Name;
                    if (CheckWeek) //new,True就是取得至星期日內
                    {
                        TitleName2 = "班級名稱： " + classInfo.Name + "　　班導師： " + tname + " 老師　　獎懲統計區間： " + startDate.ToShortDateString() + " ~ " + endDate.ToShortDateString();
                    }
                    else
                    {
                        TitleName2 = "班級名稱： " + classInfo.Name + "　　班導師： " + tname + " 老師　　獎懲統計區間： " + startDate.ToShortDateString() + " ~ " + endDate.AddDays(-2).ToShortDateString();
                    }
                    dataIndex = index + 5;
                    ...
                    //foreach (string each1 in classStudentList.Keys) //班級
                    //{
                    //    CountDetail += classStudentList[each1].Count;
                    //}
                    ...
                    while (studentCount < classStudent.Count)
                    {
                        ...
                        if (CountRows == TotleStudentRowCount && pageCount <= TotlePage)
                        {
                            ...
                            //複製 Header
                            ws.Cells.CreateRange(dataIndex, 5, false).Copy(prototypeHeader);
                    ...
                    string Help1 = "※茲公佈獎懲週報表，如有不符之處，請於三天內至【學務處－生活輔導組】核對更正，逾期不受理！";
