...
namespace K12.Behavior.Shinmin.Night
{
    internal class Report : IReport
    {
        ...
        public void Print()
        {
            ...
            WeekAbsenceReportN weekForm = new WeekAbsenceReportN();

            if (weekForm.ShowDialog() == DialogResult.OK)
            {
                #region 節次
                ConfigData cd = School.Configuration[weekForm.ConfigPeriodName];
        ...
        void _BGWAbsenceWeekListByPeriod_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            foreach (string type in AbsenceList)
            {
                ...
                columnTable_asbs.Add(type, colIndex - dayStartIndex);
                colIndex++;
                _BGWAbsenceWeekListByPeriod.ReportProgress((int)(((double)current++ * 100.0) / (double)all));
            ...
            //新民客制 - 增加缺席欄位(101/11/16)
            prototype.Worksheets[0].Cells.CreateRange(dayStartIndex, 1, true).Copy(tempEachColumn);
            prototype.Worksheets[0].Cells.CreateRange(dayStartIndex, 1, true).ColumnWidth = 3;
            prototype.Worksheets[0].Cells[titleRow + 1, dayStartIndex].PutValue("缺席");
            prototype.Worksheets[0].Cells.Merge(titleRow, dayStartIndex, 2, 1);
            columnTable.Add("缺席", dayStartIndex);

            dayStartIndex++;

            //新民客制 - 增加學生簽名欄位(1000816)
            prototype.Worksheets[0].Cells.CreateRange(dayStartIndex, 1, true).Copy(tempEachColumn);
            prototype.Worksheets[0].Cells.CreateRange(dayStartIndex, 1, true).ColumnWidth = 12;
            prototype.Worksheets[0].Cells[titleRow, dayStartIndex].PutValue("學生簽名");
            prototype.Worksheets[0].Cells.Merge(titleRow, dayStartIndex, 2, 1);
            ...
            foreach (ClassRecord classInfo in selectedClass)
            {
                if (list.Contains(classInfo.ID))
                {
                    ...
                    string TitleName1 = School.DefaultSchoolYear + " 學年度 " + School.DefaultSemester + " 學期 " + School.ChineseName + " 缺曠週報表";
                    string TitleName2 = "";

                    if (CheckWeek) //new,True就是取得至星期日內
                    {
                        TitleName2 = "班級名稱： " + classInfo.Name + "　　班導師： " + TeacherName + "　　缺曠統計區間： " + startDate.ToShortDateString() + " ~ " + endDate.ToShortDateString() + "　　列印日期：" + DateTime.Today.ToShortDateString();
                        //ws.Cells[index + 1, 0].PutValue();
                    }
                    else
                    {
                        TitleName2 = "班級名稱： " + classInfo.Name + "　　班導師： " + TeacherName + "　　缺曠統計區間： " + startDate.ToShortDateString() + " ~ " + endDate.AddDays(-2).ToShortDateString() + "　　列印日期：" + DateTime.Today.ToShortDateString();
                        //ws.Cells[index + 1, 0].PutValue();
                    }
                    dataIndex = index + 4;
                    ...
                    int TotlePage = 0;
                    //資料筆數count
                    int CountDetail = classStudentList[classInfo.ID].Count;

                    int TotleStudentRowCount = 30; //取總頁數                    
                    TotlePage = CountDetail / TotleStudentRowCount;

                    int pageCount = 1; //第一頁
                    if (CountDetail % TotleStudentRowCount != 0)
                    {
                    ...
                    while (studentCount < classStudent.Count)
                    {
                        ...
                        if (studentSemesterAbsenceList.ContainsKey(studentID))
                        {
                            ...
                            ws.Cells[dataIndex, columnTable["缺席"]].PutValue(缺席);
                        }
                        ...
                        #region 分頁
                        if (CountRows == TotleStudentRowCount && pageCount <= TotlePage)
                        {
                            CountRows = 0;
                            //分頁
                            ws.HPageBreaks.Add(dataIndex, dataIndex);
                            //複製 Header
                            ws.Cells.CreateRange(dataIndex, 4, false).Copy(prototypeHeader);
                    ...
                    string Help1 = "※茲公佈進校缺曠週報表，如有不符之處，請於三天內至【學務處－生活輔導組】核對更正，逾期不受理！";
                    ws.Cells.CreateRange(dataIndex, 0, 1, dayStartIndex).RowHeight = 20;
                    ws.Cells.CreateRange(dataIndex, 0, 1, dayStartIndex).Merge();
                    ws.Cells[dataIndex, 0].PutValue(Help1);
                    ws.Cells[dataIndex, 0].Style.HorizontalAlignment = TextAlignmentType.Left;

                    dataIndex++;

                    ws.Cells.CreateRange(dataIndex, 0, 1, dayStartIndex).RowHeight = 40;
                    ws.Cells.CreateRange(dataIndex, 0, 1, dayStartIndex).Merge();
                    ws.Cells.CreateRange(dataIndex, 0, 1, dayStartIndex).SetOutlineBorder(BorderType.BottomBorder, CellBorderType.None, Color.White);
                    ws.Cells[dataIndex, 0].PutValue("學務處蓋章：　　　　　　　　　　　　　　　　　　　　　　　　　　　　導師蓋章：");
                    ws.Cells[dataIndex, 0].Style.HorizontalAlignment = TextAlignmentType.Left;
